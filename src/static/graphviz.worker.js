...
self.addEventListener(
  ...
  function (event) {
    ...
    if (eventType === 'INIT') {
      console.log(`worker: fetching wasm binary using ${WASM_PATH}`);
      fetch(WASM_PATH)
        .then(res => {
          if (!res.ok)
            throw Error(
        ...
        .then(bytes => {
          console.log('worker: extracting wasm binary');
      ...
      graphvizWebAssemblyBinaryPromise.then(graphvizWebAssemblyBinary => {
        console.log('worker: processing dot graph using graphviz');
        // console.log(dot);
