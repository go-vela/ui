...
explainPipelineConfig :
    { baseUrl : String
    , session : Auth.Session.Session
    , onResponse : Result (Http.Detailed.Error String) ( Http.Metadata, Vela.PipelineExplanation ) -> msg
    , org : String
    , repo : String
    , ref : String
    }
    -> Effect msg
explainPipelineConfig options =
    Api.try
        options.onResponse
        (Api.Operations.explainPipelineConfig
            options.baseUrl
            options.session
            options
        )
