...
context('Builds', () => {
  ...
  context('logged in and server returning 5 builds', () => {
    ...
    it('cancel build button should be present when running', () => {
      cy.get('@firstBuild').should('exist').find('[data-test=cancel-build]').should('exist');
      cy.get('@firstBuild').next().should('exist').find('[data-test=cancel-build]').should('not.exist');
      cy.get('@firstBuild').next().next().should('exist').find('[data-test=cancel-build]').should('not.exist');
      cy.get('@firstBuild').next().next().next().should('exist').find('[data-test=cancel-build]').should('not.exist');
      cy.get('@firstBuild').next().next().next().next().should('exist').find('[data-test=cancel-build]').should('exist');
    });

    it('build menu should expand and close when action is fired', () => {
      cy.get('@firstBuild').should('exist').find('[data-test=cancel-build]').should('not.be.visible');
      cy.get('@firstBuild').should('exist').find('[data-test=restart-build]').should('not.be.visible');
      cy.get('@firstBuild').should('exist').find('[data-test=build-menu]').click();
      cy.get('@firstBuild').should('exist').find('[data-test=cancel-build]').should('be.visible');
      cy.get('@firstBuild').should('exist').find('[data-test=restart-build]').should('be.visible');
      cy.get('@firstBuild').should('exist').find('[data-test=cancel-build]').click();
      cy.get('@firstBuild').should('exist').find('[data-test=cancel-build]').should('not.be.visible');
      cy.get('@firstBuild').should('exist').find('[data-test=restart-build]').should('not.be.visible');
    });
